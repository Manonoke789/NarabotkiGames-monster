...
namespace Gley.DailyRewards.Internal
{
    public class TimerButtonManager : MonoBehaviour
    {
        ...
        /// <param name="buttonID"></param>
        /// <returns></returns>
        public TimeSpan GetTimeToPass(TimerButtonIDs buttonID)
        {
            var timerProperty = timers.FirstOrDefault(cond => cond.buttonID == buttonID);
            if (timerProperty == null)
            {
                Debug.LogError("Timer with ID " + buttonID + " is not initialized.");
                return TimeSpan.Zero;
            }
            return timerProperty.timeToPass;
        }
        ...
        public string GetRemainingTime(TimerButtonIDs buttonID)
        {
            ...
            {
                return string.Format("{1:D2}:{2:D2}", (int)difference.TotalHours, difference.Minutes, difference.Seconds);
            }
